document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('addCustomPizza').addEventListener('click', addCustomPizzaToCart);
});
...
function addCustomPizzaToCart() {
    const checkedBoxes = document.querySelectorAll('input[name="customizations"]:checked');
    const customizations = Array.from(checkedBoxes).map(cb => parseInt(cb.value));
    ...
    axios.post('/api/cart', {
        ...
        document.getElementById('cart').textContent = `Cart(${response.data.cartCount})`;
